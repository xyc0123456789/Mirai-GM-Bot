...
@Slf4j
@Component
public class DetectMembers extends AbstractGroupMessageApi {
    ...
    public boolean condition(MessageEventContext messageEventContext) {
        if(!"#detect".equalsIgnoreCase(messageEventContext.getContent())){
            return false;
        }
    ...
    public CommonResponse handler(MessageEventContext messageEventContext) {
        if (messageEventContext.getMessageEvent().getSubject().getId()== GroupId.DEV){
            ...
            membersService.detect(messageEventContext.getMessageEvent().getSubject().getId(), DateFormateUtil.getOffsetDate000000(1));
        }
        return null;
    }
