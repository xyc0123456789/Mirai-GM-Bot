...
@Slf4j
@Component
public class DetectForceMembersRandom extends AbstractGroupMessageApi {
    ...
    public boolean condition(MessageEventContext messageEventContext) {
        if (!"#detectforcer".equalsIgnoreCase(messageEventContext.getContent())){
            return false;
        }
    ...
    public CommonResponse handler(MessageEventContext messageEventContext) {
        if (messageEventContext.getMessageEvent().getSubject().getId()== GroupId.DEV){
            ...
            membersService.detect(messageEventContext.getMessageEvent().getSubject().getId(), DateFormateUtil.getOffsetDate000000(1000), 20);
        }
        return null;
    }
    ...
    public String commandName() {
        return "member.detect.force.random";
    }
}
