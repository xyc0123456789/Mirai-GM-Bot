...
@Slf4j
@Component
public class TempMessagePermissionBlock extends AbstractGroupMessageApi{
    ...
    public boolean condition(MessageEventContext messageEventContext) {
        if (!messageEventContext.getContent().trim().startsWith("#temppermissionblock")){
            return false;
        }
    ...
    public CommonResponse handler(MessageEventContext messageEventContext) {
        ...
        for (Long id:ids){
            TempGroupSpecialList specialList = tempGroupSpecialListService.getAndUpdateOrAddOne(groupId, id, 0L);
            if (specialList!=null){
        ...
        MessageChain messageChain = new MessageChainBuilder().append(quoteReply).append("#temppermissionblock success ids:").append(success.toString()).build();
    ...
    public String commandName() {
        return "temp.message.give.permission";
    }
}
