...
import com.king.db.service.TempGroupSpecialListService;
...
@Slf4j
@Component
public class TempMessagePermissionAllow extends AbstractGroupMessageApi{
    ...
    public boolean condition(MessageEventContext messageEventContext) {
        if (!messageEventContext.getContent().trim().startsWith("#temppermissionallow")){
            return false;
        }
    ...
    public CommonResponse handler(MessageEventContext messageEventContext) {
        ...
        String content = messageEventContext.getContent().trim().substring(20);
        ...
        for (Long id:ids){
            TempGroupSpecialList specialList = tempGroupSpecialListService.getAndUpdateOrAddOne(groupId, id, 1L);
            if (specialList!=null){
        ...
        MessageChain messageChain = new MessageChainBuilder().append(quoteReply).append("#temppermissionallow success ids:").append(success.toString()).build();
    ...
    public String commandName() {
        return "temp.message.give.permission.allow";
    }
}
